...
directions=[(-16,-32,-15,-17),(1,2,-15,17),(16,32,15,17),(-1,-2,-17,15),(-33,33,-31,31,18,-18,14,-14),(-17,-15,17,15),(1,-1,16,-16),(1,-1,16,-16,-17,-15,17,15),(1,-1,16,-16,-17,-15,17,15)]
        ...
        for key in extras.VALID_KEYS:
            ...
            for location in directions[self.board[key][1]]:
                for keyr in count(key + location, location):
                    ...
                    if self.board[keyr] in (0,1,2,3) and location in (-16,16,1,-1,-32,32,2,-2) and captured: break
                    if self.board[keyr] in (0,1,2,3) and location in (-32,32,2,-2) and (key not in pstart or self.board[keyr-double_pawns[self.color]])!=0: break
                    if self.board[keyr] in (0,1,2,3) and location in (-17,-15,17,15) and not captured: break
                    ret.append((key,keyr))
                    if self.board[keyr] in (0,1,2,3,4,8) or captured: break
        return ret
    def nullmove(self):
...
class Search:
    ...
    def search(self,pos,alpha,beta,depth=3,ply=1):
        ...
        o_alpha=alpha
        best=-infinity
        if len(pos.gen_moves())<2: depth+=1
        for move in self.sorted(pos):
    ...
    def sorted(self,pos):
        ...
        for move in sorted(pos.gen_moves(),key=pos.value,reverse=True):
            if move==killer: continue # If this move matches the killer then continue, we have already yielded the killer move.
            yield move
def main():
    search=Search()
